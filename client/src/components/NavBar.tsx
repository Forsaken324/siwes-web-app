  ...
  return (
    <nav className="fixed top-0 left-0 z-50 w-full flex items-center justify-between md:justify-evenly border border-b-light-gray dark:border-gray-700 border-t-0 border-r-0 border-l-0 bg-white dark:bg-black py-3 md:py-0">
      ...
      <Link to={"/"} className="flex items-center dark:text-white">
        ...
        <p className="font-bold text-[25px] md:text-[30px] text-black dark:text-white">
          rentals
        </p>
      </Link>

      <div
        className={`max-md:absolute max-md:bg-gray-900 max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg z-50 flex flex-col md:flex-row items-center max-md:justify-center gap-8 min-md:px-8 py-3 max-md:h-screen overflow-hidden transition-[width] duration-300 dark:text-white ${
      ...
      >
        ...
        <div
          className={`flex items-center gap-7 ${
            menuOpen ? "text-white flex-col-reverse" : ""
          }`}
        >
          <div className="flex w-[233px] h-[38px] items-center rounded-full border border-light-gray dark:bg-gray-700">
            <input
              type="text"
              placeholder="Search cars"
              className="outline-none w-[185px] ml-2 dark:text-white"
              value={searchValue}
      ...
      <div className="flex items-center gap-5">
        {theme == "light" ? (
          <SunIcon className="dark:text-white" onClick={changeTheme} />
        ) : (
          <MoonIcon className="dark:text-white" onClick={changeTheme} />
        )}
        {isLoggedIn ? (
          <User />
        ) : (
